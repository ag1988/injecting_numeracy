...
def read_squad_examples(input_file, is_training, version_2_with_negative, num_train_samples=-1):
    ...
    for entry in input_data:
        for paragraph in entry["paragraphs"]:
            ...
            for qa in paragraph["qas"]:
                ...
                if is_training and num_train_samples > 0 and len(examples) >= num_train_samples:
                    done = True
    return examples
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
                                 doc_stride, max_query_length, is_training, indiv_digits=False):
    ...
    for (example_index, example) in enumerate(examples):
        query_tokens = tokenize(example.question_text)
        ...
        if is_training and not example.is_impossible:
            ...
            (tok_start_position, tok_end_position) = _improve_answer_span(
                ...
                example.orig_answer_text, indiv_digits)
...
def _improve_answer_span(doc_tokens, input_start, input_end, tokenizer,
    ...
    tok_answer_text = " ".join(tokenize(orig_answer_text))
